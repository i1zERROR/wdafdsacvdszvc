...
let owners = '1105231113024196703'//ايدي الي هيتحول 
let owner;
const
  updateAdmins = () => {
  owner = db.get("admins")
  owners = '1105231113024196703'
  owner.unshift('1105231113024196703')//اي دي حقك
}
updateAdmins()
...
client.on('guildCreate', guild => {
  ...
  let textList = [`${prefix}Stock`, `${prefix}help` , `${prefix}buy`] 
...
`)
    ...
    if (command === "buy") {
      ...
      if (!message.channel.name.startsWith('ticket-')) {
        message.reply("**YOU HAVE TO OPEN A TICKET TO BUY. ❌**");
        return;
      }
      ...
      let buys = new Discord.MessageEmbed()
        ...
        .setTimestamp()
        .addField(`**شراء ${account} :**`
          ,
          `
            
            > **-العدد  :** \`${numOfAcc}\`
            > **-المبلغ الكلي :** \`${price * numOfAcc}\`  
...
#credit <@${owners}> ${price} \`\`\``   )
      ...
      message.channel.send({ embeds: [buys] }).then(msg => {
        ...
        const filter = ({ content, author: { id } }) => {
          ...
          return content.startsWith(
            ...
            id === probotid &&
            (Number(content.slice(content.lastIndexOf("`") - String(price * numOfAcc).length, content.lastIndexOf("`"))) >= price * numOfAcc)
        }
  ...
  message.channel.send({embeds : [embed]})
          ...
          //mm
                ...
                .send(`**<@${message.author.id}> bought a ${account} account \n ${price} **`);
            }
            }
          })
...
//4
  ...
  if (command === "stock") {
      ...
      let embed = new Discord.MessageEmbed()
        ...
        .setTimestamp()
        .setColor("BLUE");
        for (let { name, price, disc, tax, emails } of accounts) {
